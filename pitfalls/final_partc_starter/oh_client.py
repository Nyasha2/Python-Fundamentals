...
This program simulates office hour sessions in which office hour entries
are resolved and their information updated and stored in files.
"""
...
def log_resolved_questions(oh_manager, resolved_filename):
    ...
    """
    resolved = oh_manager.get_resolved_entries()
    with open(resolved_filename, 'w') as file:
        file_writer = csv.DictWriter(file, RESOLVED_QUESTION_LOG_COLUMNS)
        file_writer.writeheader()
        for entry in resolved:
            row = {
                'assignment': entry.get_assignment(),
                'question': entry.get_question(),
                'response_time': entry.get_response_time(),
                'response_ta': entry.get_response_ta().name,
                'ta_timeframe': entry.get_response_ta().get_oh_time_frame(),
            }
...
def log_unresolved_questions(oh_manager, unresolved_filename):
    ...
    """
    unresolved = oh_manager.get_unresolved_entries()
    with open(unresolved_filename, 'w') as file:
        writer = csv.DictWriter(file, UNRESOLVED_QUESTION_LOG_COLUMNS)
        writer.writeheader()
        for entry in unresolved:
            row = {
                'student_uid': entry.get_student().get_uid(),
                'student_name': entry.get_student().get_name(),
                'student_email': entry.get_student().get_email(),
                'assignment': entry.get_assignment(),
                'question': entry.get_question()
            }
            writer.writerow(row)
