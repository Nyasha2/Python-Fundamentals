...
def create_SC(element, lparam):
    ...
    lattice = CubicLattice(lparam, atoms)
    return lattice
...
def create_BCC(element, lparam):
    ...
    Returns:
        - (CubicLattice) - CubicLattice in a BCC form comprised of `element`
          Atoms and having the passed lattice parameter.
    """
    center_atom = Atom(element, (0.5 * lparam, 0.5 * lparam, 0.5 * lparam))
    atoms = get_corner_atoms(element, lparam)
    atoms.append(center_atom)
    lattice = CubicLattice(lparam, atoms)
    return lattice
...
def create_FCC(element, lparam):
    ...
    Returns:
        - (CubicLattice) - CubicLattice in a FCC form comprised of `element`
          Atoms and having the passed lattice parameter.
    """
    # TODO: D.3. (hint: the list of atoms you'll pass to CubicLattice will
    # be 8 corner Atoms + 6 face Atoms)
    atoms = get_corner_atoms(element, lparam)
    faces = [(0, 0.5, 0.5), (0.5, 0.5, 0), (0.5, 0, 0.5),
             (0.5, 0.5, 1), (1, 0.5, 0.5), (0.5, 1, 0.5)]
    for (x, y, z) in faces:
        scaled_face = (x * lparam, y * lparam, z * lparam)
        atoms.append(Atom(element, scaled_face))
    lattice = CubicLattice(lparam, atoms)
    return lattice
i